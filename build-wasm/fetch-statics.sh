...
# Single self-contained tag (sprint 6.3): the pack already CONTAINS the patch-004
# GC-fuse libmono-ee-interp.a (sha-identical to the old gcfuse-interp-004 overlay).
# The two-tag overlay mechanism is gone; when bumping to a new upstream
# FNA-WASM-Build run, re-apply patches/004 in that build (or re-bake the lib via
# mirror-statics.sh) BEFORE publishing the release — a fuse-less runtime pack
# reintroduces the ~frame-976 LMF-chain GC freeze.
STATICS_TAG="${STATICS_TAG:-toolchain-26933625837-fused004}"
